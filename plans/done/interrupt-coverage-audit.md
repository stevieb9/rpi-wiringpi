...
B2: ✅ DONE 2026-06-06 (VALIDATED, no code change) — `_pin_registration()` already locks+fetches exactly once at entry (`Core.pm:262-263`); all branches use that single local `$meta`, the only in-lock helper (`pin_to_gpio`) touches no meta, and no caller holds a lock before calling in. The apparent redundancy B2 flagged was the pre-F2 mix of `$self->{meta}` refs, which the F2 fix (commit c50f8f8 / V2) already normalized to the single `$meta`. Nothing left to collapse.

B4: ✅ RESOLVED UPSTREAM 2026-06-06 — `WiringPi::API::BackgroundInterrupts` (plural `background_interrupts` handle) inherited `read`/`fh` from the base `BackgroundInterrupt` class but its `_new` calls `SUPER::_new($pid)` without a `results_fh`, so both returned a silent `undef` — the surface advertised a results channel the shared-child handle was never designed to deliver. Intent (confirmed from the source: class comment, POD, and the absence of any `{results}` opt) is that the plural form trades results away for the one-child + arm/disarm model; results belong to the singular `background_interrupt(..., {results=>1})` and to `worker()`. **Fix (Option A, in `~/repos/wiringpi-api` branch `3.18`):** override `read`/`fh` on the plural class to croak with a pointer to the singular API instead of returning undef; documented the no-results contract in POD + Changes; added two unit assertions to `t/75-interrupts.t` (47 tests PASS, `t/85-worker.t` still PASS — `Worker` keeps the inherited results channel). Not building a multi-pin results channel (undesigned feature, no demux, no opt). (Surfaced as D2 during V13; V13 verifies edge-flow + arm/disarm gating via temp-file IPC.)
...
- (was B3) Wiring or removing the hardcoded `$signal_debug = 0` toggle in `WiringPi.pm:20` — dead but harmless; leaving as-is (user decision 2026-06-06).
