...
> **NEXT ACTION:** V12 — RPi::I2C: add HW-free `new()` addr validation + `_set_reg` defaulting; characterize-then-pin F7 (`read_bytes` returns only the last byte) and F8 (`write_word` reg/value swapped) with stubbed XS; flag the `process()` arg-order + unenforced 32-byte cap. (V8 PAUSED until a DS3231 is online.)
> **LAST SESSION:** 2026-06-24 — V11 done: added ungated `rpi-wiringpi/t/119-restore_pin_alt.t` (17 tests) covering every RP1/legacy branch of `_restore_pin_alt` (the alt-31 cleanup divergence) via mocked pi_rp1_model/pinMode/pinModeAlt/system. No bug found; `_class_signal_handler` chaining deferred to B17. (V10: core validation.)
> **ARCHIVE:** See test-coverage-gaps-archive.md for completed V1-V7, V9-V11
