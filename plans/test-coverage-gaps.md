...
- **F7** ⏸ DEFERRED → B18 (pinned V12): rpi-i2c `read_bytes` returns only the LAST byte (`$retval = (0<<8) | …` overwrite), not the documented array. `t/10-validation.t` + mirror `t/250` pin the current behaviour; the fix is a breaking contract change (→B18).
- **F8** ⏸ DEFERRED → B18 (pinned V12): rpi-i2c `write_word($self,$reg,$value)` argument order is inverted vs `write_byte` and its own POD. Pinned by t/10 + t/250; fix is a breaking contract change (→B18).
