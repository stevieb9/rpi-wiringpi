...
> **NEXT ACTION:** V11 — rpi-wiringpi: cover the Pi-5/RP1 `Core::_restore_pin_alt` cleanup branch (INPUT/OUTPUT via pinMode, alt-31 via pinctrl, real alts via pinModeAlt) by mocking pi_rp1_model/pinMode/system; plus `_class_signal_handler` prior-handler chaining. (V8 PAUSED until a DS3231 is online.)
> **LAST SESSION:** 2026-06-24 — V10 done: added ungated `rpi-wiringpi/t/118-core_validation.t` (19 tests) — factory/Meta/pin_to_gpio/pin_map validation + servo/pwm non-root guards, built off-board via setup=>'none'. No bug found; the non-HW-free bits (pin() collision, device smoke, meta lifecycle) deferred to B16. (V9: wiringpi-api F5 + validation.)
> **ARCHIVE:** See test-coverage-gaps-archive.md for completed V1-V7, V9-V10
