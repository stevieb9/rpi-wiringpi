...
> **LAST SESSION:** 2026-06-24 — V12 done: built rpi-i2c, added `t/10-validation.t` (7) + mirror `t/250-i2c_unit.t` (7) — addr validation, _set_reg, and F7/F8 pinned (read_bytes/write_word contract bugs → deferred to B18). User renamed the dist 3.19→3.1802 mid-task; rebuilt + reinstalled. (V11: _restore_pin_alt.)
...
|---|---|---|---|
| RPi::ADC::ADS | t/400,405,420,421,410,425,435,445 (HW) | 22 (incl. HW-free bit tests) | V5 |
| RPi::ADC::MCP3008 | t/410,435 (HW) | boilerplate only | V13 |
| RPi::DAC::MCP4922 | t/410 (HW) | boilerplate only | V4 |
| RPi::DigiPot::MCP4XXXX | t/445 (HW) | bytes.t, init.t | V3 |
| RPi::GPIOExpander::MCP23017 | t/355,350 (HW) | 17 (HW-gated) | V6 |
| RPi::RTC::DS3231 | t/530,**532-rtc-bcd** (HW+BCD) | 19 | V8 |
| RPi::EEPROM::AT24C32 | **t/540,541,542** (mirrored, RPI_EEPROM-gated) | boilerplate (moved here) | V7 |
| RPi::Pin | t/105-108 + most pin tests (HW) | 7 | V2 |
| RPi::Const | constants used throughout | 15 | V1 |
| RPi::BMP180 | t/531-bmp (HW) | boilerplate | V19 |
| RPi::I2C | t/605-i2c (HW, Arduino) | boilerplate | V12 |
| RPi::LCD | t/620-lcd (HW) | boilerplate | V20 |
| RPi::OLED::SSD1306 | **t/500-509,520** (mirrored, RPI_OLED-gated, return-value asserts) | boilerplate | V21 |
| RPi::Serial | t/610-serial (HW) | boilerplate | V15 |
| RPi::StepperMotor | **t/350** (HW: real motor — engine physically validated), t/351 (StepperSeek unit) | boilerplate | V16 |
| RPi::SysInfo | **t/300-309** (mirrored, RPI_BOARD-gated) | 14 (t/50,55 ungated seam) | V22 |
...
|----|------|---------|----------|--------|
| V8 | **RPi::RTC::DS3231** — integration-covered by t/530, and BCD is mirrored in `t/532-rtc-bcd` (HW-free). Gap: extend `532` to full 0-99 `dec2bcd`/`bcd2dec` round-trip + field maxes; extract & unit-test the temp decode incl. the negative-temp sign path (never executed); add HW-free range-validation croaks for every setter (mock-fd, B3). | `cd ~/repos/rpi-wiringpi && prove -Iblib/lib -Ilib t/532-rtc-bcd.t` | Full BCD range + negative temp + setter validation pass HW-free. | ⏳ |
...
| V15 | **RPi::Serial** — integration-covered by t/610 (HW). Own repo boilerplate. The pure-Perl framing stack is fully HW-free: `crc`/`crc16` known vectors (incl. empty), `tx` frame+CRC-split order, `rx` stateful reassembly + CRC-mismatch warn + pre-start discard (override getc/avail), `write` undef croak + >255 wrap. F14 (`new()` no croak on failed open → fd=-1; baud unvalidated). Mirror here. | `cd ~/repos/rpi-serial && prove -Ilib t/` | crc/tx/rx framing + write validation covered HW-free. | ⏳ |
| V16 | **RPi::StepperMotor** — the step **engine is already physically validated by t/350** (real motor + magnet timing); t/351 unit-tests the StepperSeek helper. Own repo boilerplate. Gap = StepperMotor's OWN unit logic, HW-free via an injected mock `expander` recording (pin,HIGH/LOW): cw pattern + wrap, ccw reversal + reverse-wrap, `_turns` rounding (incl. `cw(1)`→0), `_pins` 4-elem + `new` missing-`pins` croaks. F10 (`speed()` validation inverted dead code — invalid silently accepted). Mirror here. | `cd ~/repos/rpi-steppermotor && prove -Ilib t/` | step-sequence + _turns + validation covered HW-free; speed() bug fixed. | ⏳ |
...
| V22 | **RPi::SysInfo** — well covered: t/300-309 here (RPI_BOARD-gated) + own t/50,55 (ungated seam). Gap: `cpu_percent`/`mem_percent` wrappers have ZERO ungated coverage though the XS runs on any Linux; replay the OO `$sys->method` form (core_temp/gpio_info/pi_model/pi_details) through the seam (HW-only today); 16GB revision decode; F15 (`_format` doesn't clamp the XS `-1.0` sentinel → "-1.00"); fix the malformed `raspi_config` blank-line regex. Mirror the ungated seam tests here. | `cd ~/repos/rpi-sysinfo && prove -Ilib t/` | cpu/mem wrappers + OO path + sentinel/regex defects covered HW-free. | ⏳ |
| V24 | **Audit the FAQ "RUNNING TESTS" section** for content that's wrong or missing (the per-test table is now generated by `scripts/gen-faq-test-table.pl`, but the surrounding hand-authored prose is not). Known-stale already found: (a) the "Setup and configuration" intro points at `docs/test-platform/README` for "the GPIO pins" — the README deliberately carries NO pin data (pins live in `test-pinout-doc.md`) — and links the legacy single-breadboard diagram (`brewbuild_test_platform_bb.jpg`) instead of the current multi-board pinout images/schematic; (b) `t/420-adc_samples.t` + `t/421-adc_gain.t` are board-2 HW tests but lack the `RPI_BOARD_2` convenience BEGIN block, so the generated table shows them as board `-` (reconcile: add the block, or confirm they're intentionally board-independent). Then sweep the rest of the section (env-var prose, RPI_* descriptions, board/rig-runner references, `t/scripts/` links, the `RPI_OBJECT_COUNT`/`RPI_PIN_COUNT` notes) for further drift vs `t/` and `docs/test-platform/`. | manual review of `lib/RPi/WiringPi/FAQ.pod` RUNNING TESTS section cross-checked against `t/*.t` + `docs/test-platform/`; `perl scripts/gen-pod-md.pl` to re-render | FAQ test section accurate + complete; README/diagram refs corrected; 420/421 board gating reconciled; no remaining stale prose. | ⏳ |
...
- **F6** ⏸ DEFERRED → B15 (surfaced V7): rpi-eeprom-at24c32 `eeprom_init` returns -1 on open failure (after a `fprintf(stderr)`) but `new()` swallows it (no croak/warn) — a bad device path builds a broken `fd == -1` object. Documented by `rpi-wiringpi/t/543-eeprom_validation.t`; the fix (croak on -1) is a `new()` contract change, deferred to B15.
- **F7** ✅ RESOLVED (rpi-i2c-fixes V8; was ⏸ DEFERRED → B18, pinned V12): rpi-i2c `read_bytes` returns only the LAST byte (`$retval = (0<<8) | …` overwrite), not the documented array. `t/10-validation.t` + mirror `t/250` pin the current behaviour; the fix is a breaking contract change (→B18). [fixed 2026-07-07 in rpi-i2c 3.1803: returns the documented array ascending from the base register; pins flipped in both test files]
- **F8** ✅ RESOLVED (rpi-i2c-fixes V8; was ⏸ DEFERRED → B18, pinned V12): rpi-i2c `write_word($self,$reg,$value)` argument order is inverted vs `write_byte` and its own POD. Pinned by t/10 + t/250; fix is a breaking contract change (→B18). [fixed 2026-07-07 in rpi-i2c 3.1803: now ($value, [$reg]) with the _set_reg default; pins flipped in both test files]
...
B18 — ✅ RETIRED 2026-07-07, executed as rpi-i2c-fixes V8, shipped in rpi-i2c 3.1803 (last-digit version bump marks the API break per its D2): rpi-i2c — coordinated API-contract fix (the bugs V12 pinned): `read_bytes` should accumulate N bytes into an array (it overwrites `$retval` and returns only the base-register byte — F7); `write_word` should take `($value, [$reg])` to match `write_byte` + its own POD (currently `($reg, $value)` — F8); `process()`'s code vs POD arg order disagree; and `write_block`'s documented 32-byte cap is unenforced. All breaking changes, so coordinated + deferred; `t/10-validation.t` + `t/250-i2c_unit.t` pin the current behaviour so the fix will flag them.
...
B15: rpi-eeprom-at24c32 — make `new()` croak (or at least warn + return undef) when `eeprom_init` returns -1, instead of building a broken `fd == -1` object (F6). It's a `new()` contract change, so it's deferred rather than slipped into a test task; `t/543` currently documents the swallow.
