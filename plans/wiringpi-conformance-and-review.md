...
| V6 | **rpi-bmp180** — audit constant source (F5) + `:all` bmp180_*/setup_gpio calls; rebuild XS + test | `cd ~/repos/rpi-bmp180 && perl Makefile.PL && make && PI_BOARD=1 RPI_OBJECT_COUNT=0 prove -blv t` | Compiles; tests pass/skip | ⏳ |
| V7 | **rpi-pin** — audit inherited parent methods + `WiringPi::API::background_interrupt()` call vs reworked interrupt subsystem (F6); build + test | `cd ~/repos/rpi-pin && perl Makefile.PL && make && PI_BOARD=1 RPI_OBJECT_COUNT=0 prove -blv t` | Interrupt handle usage matches new API; tests pass/skip | ⏳ |
| V8 | **rpi-lcd** — audit `use parent WiringPi::API` methods + RPi::Const; build + test | `cd ~/repos/rpi-lcd && perl Makefile.PL && make && PI_BOARD=1 RPI_OBJECT_COUNT=0 prove -blv t` | Compiles; tests pass/skip | ⏳ |
| V9 | **rpi-steppermotor** — audit `:perl` (setup_gpio/write_pin/pin_mode) + RPi::Const; build + test | `cd ~/repos/rpi-steppermotor && perl Makefile.PL && make && PI_BOARD=1 RPI_OBJECT_COUNT=0 prove -blv t` | Compiles; tests pass/skip | ⏳ |
| V10 | **rpi-gpioexpander-mcp23017** — confirm all RPi::Const constants it imports (MCP23017_*, HIGH) still exist; rebuild XS + test | `cd ~/repos/rpi-gpioexpander-mcp23017 && perl Makefile.PL && make && PI_BOARD=1 RPI_OBJECT_COUNT=0 prove -blv t` | Constants resolve; tests pass/skip | ⏳ |
| V11 | **rpi-dht11 + rpi-hcsr04** — own XS links libwiringPi 3.18; confirm core C funcs (wiringPiSetupGpio/pinMode/digitalWrite/digitalRead) still present; build + test each | per-repo `perl Makefile.PL && make && PI_BOARD=1 RPI_OBJECT_COUNT=0 prove -blv t` | Both compile against wiringPi 3.18; tests pass/skip | ⏳ |
| V12 | **rpi-oled-ssd1306** — includes `wiringPiI2C.h`; confirm builds against wiringPi 3.18; test | `cd ~/repos/rpi-oled-ssd1306 && perl Makefile.PL && make && PI_BOARD=1 RPI_OBJECT_COUNT=0 prove -blv t` | Compiles; tests pass/skip | ⏳ |
| V13 | **Independent sweep** — rpi-adc-ads, rpi-eeprom-at24c32, rpi-i2c, rpi-rtc-ds3231, rpi-serial, rpi-sysinfo: confirm no WiringPi::API/libwiringPi dependency and each builds + tests clean (confirm-only, likely no edits) | per-repo `perl Makefile.PL && make && prove -bl t` (+ device `RPI_*` gates) | No wiringPi coupling; all build/test clean | ⏳ |
| V14 | **rpi-const** — the 19th sibling and the dependency root the whole migration single-sources from (F44); confirm-only: repo `lib/RPi/Const.pm` version == installed, and it exports every name/value consumers use (debate verified 0 drift today) | `cd ~/repos/rpi-const && perl Makefile.PL && make && prove -bl t` | Builds/tests clean; no drift vs installed RPi::Const | ⏳ |
| V23 | **HIGH-severity code bugs** in rpi-wiringpi Core.pm/WiringPi.pm: `gpio_layout()` infinite self-recursion (F16), `pwm_mode()` getter always croaks (F17), `_pwm_in_use` `&&`→`||` guard leaks PWM state (F18), `cleanup` deletes shared pin for all co-owners (F19), **`WiringPi.pm:38` `RPI_MODE_WPI==0` is falsy so a 2nd `new()` silently re-inits to GPIO (F42)** — use `defined` like `Core.pm:97-98` | `cd ~/repos/rpi-wiringpi && perl -Ilib -e 'use RPi::WiringPi'` + `PI_BOARD=1 RPI_OBJECT_COUNT=<n> prove -bl t/105* t/106* t/109* t/140* t/110* t/150*` | All five bugs fixed; affected tests pass | ⏳ |
...
- **F6** (→V7): `rpi-pin/lib/RPi/Pin.pm:~128` calls `WiringPi::API::background_interrupt(...)`; interrupt subsystem was reworked (split BackgroundInterrupt classes, new return-handle semantics) — verify call site matches new contract. **Verified-at-source (debate):** `Pin.pm:128,155` `set_interrupt`/`background_interrupt` call sites match the new API (still confirm under build/test in V7).
- **F7** (→V13/backlog): several siblings declare an old `WiringPi::API` minimum (2.36xx) in Makefile.PL; satisfied numerically by 3.1802 but understates the real requirement. Tracked as B1 (out of strict scope).
...
- **F16** [HIGH] (→V23): `Core.pm:18-20` `gpio_layout()` calls `$_[0]->gpio_layout` — unbounded self-recursion; never reaches `WiringPi::API`'s impl. Use `SUPER::gpio_layout`/fully-qualified call.
- **F17** [HIGH] (→V23): `Core.pm:141-147` `pwm_mode()` getter form croaks — when `$mode` is undef the `else` fires. POD documents it as set/get; only validate when defined.
- **F18** [HIGH] (→V23): `Core.pm:379` `_pwm_in_use` early-returns on `! pins && ! register` where every sibling guard uses `|| `. With `rpi_register_pins=>0, rpi_register=>1` it still writes `pwm.users` but `cleanup` (gated on `_rpi_register_pins`) never clears it → stale PWM-in-use leak. Match the `||` form.
- **F19** [med→HIGH] (→V23): `Core.pm:223-229` `cleanup` deletes a shared pin from meta for ALL co-owners once the calling object is one user, silently dropping other uuids' registration (the PWM path above does it correctly per-uuid). Delete only `{users}{$self->uuid}`, reset/remove pin only when no users remain.
