...
> **NEXT ACTION:** V29 — test correctness fixes (F11/F12/F33/F34/F35). (V24 version bump is 🤚 USER-owned — skip it.)
> **LAST SESSION:** V28 done — all 7 POD source fixes: F26 INTERRUPTS.pod paths→docs/examples/; F27 bareword constructor in Meta SYNOPSIS + 2 FAQ sites (one more than the finding caught); F28 `my gps`/`$ruler`; F29 =head1 AUTHOR in Meta; F30 oled() 3rd param documented (default true per SSD1306); F31 VALIDATED — POD 0=MS/1=BAL matches constants (validation now uses PWM_MODE_MS/BAL for clarity); F32 caveats updated — RPi::Pin 2.3609 (required) ships background_interrupt. README + all md regenerated; stale text verified gone. POD tests (t/500/505/510, 26 tests) + t/104 pass. Uncommitted.
> **ARCHIVE:** See wiringpi-conformance-and-review-archive.md for completed V1-V14, V20-V23, V25-V28, V31
