...
> **NEXT ACTION:** V27 — regenerate README from WiringPi.pm POD (F25) + run `perl scripts/gen-pod-md.pl` to refresh stale docs/pod md (F14). (V24 version bump is 🤚 USER-owned — skip it.)
> **ADDENDUM (post-V26, user-requested):** regression tests added for all V23/V25/V26 fixes — new t/104-core_regressions.t (F16/F17/F18/F42/F10/F24) + t/154-fatal_exit.t (F23 incl. forked SIGINT-termination case); t/03 gained die-in-lock + missing-slot blocks (F20/F21); t/153 gained CODE/'IGNORE'/multi-object restore cases (F22); both new files in MANIFEST (manicheck clean). Full t/0-2* sweep: 1389 tests PASS (t/104 also run as root for the pwm-getter branch). Note: first version of the t/03 additions left `storage` in shm and tripped t/01's reset assertion — fixed with a trailing meta_erase(1) + one-time segment wipe.
