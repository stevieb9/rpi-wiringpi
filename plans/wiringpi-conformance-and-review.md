...
> **NEXT ACTION:** V28 — POD source fixes (F26-F32), then re-run README/md regen per the row note. (V24 version bump is 🤚 USER-owned — skip it.)
> **LAST SESSION:** V27 done — README regenerated via pod2text (F25: __DIE__ claim, 2.36 ref, missing methods/params all fixed; V26's fatal_exit clarification flowed through) + gen-pod-md.pl refreshed docs/pod/WiringPi.md (serial NOTE) and FAQ.md (dtparam typo) — only the two F14-flagged files changed, others were in sync. Changes updated. NOTE: README still carries the F32 RPi::Pin-2.3608 caveat from POD source — V28 fixes it at source then re-regens. Regression-test addendum from post-V26 stands (1389 tests green). Uncommitted.
> **ARCHIVE:** See wiringpi-conformance-and-review-archive.md for completed V1-V14, V20-V23, V25-V27, V31
> **LAST SESSION:** V26 done — robustness batch: F20 `_meta_txn` guaranteed-unlock wrapper, all 8 critical sections converted (Meta erase/delete/get/set, Core cleanup/_pin_registration/_pwm_in_use/unregister_object, WiringPi new); F21 conditional-`my` split; F22 handlers restored on last-object cleanup incl. non-CODE dispositions + closure leak freed (t/153 updated to new contract); F23 fatal_exit strictly per-object (any 0 → survive; POD clarified); F24 `_led_cmd` system+warn. Changes: 5 entries. Verified: 213-test subset + 661 (t/0-1*) + 626 (t/2*) pass; live checks of F20/F21/F22/F23/F24 all green. Uncommitted.
> **ARCHIVE:** See wiringpi-conformance-and-review-archive.md for completed V1-V14, V20-V23, V25, V26, V31
...
| V28 | **POD source fixes** — broken example-doc paths in INTERRUPTS.pod/FAQ.pod (F26), invalid `RPi::WiringPi;` constructor in Meta.pm/FAQ (F27), `my gps`/`$ruler`/`$sensor` example errors (F28), Meta.pm missing `=head1 AUTHOR` (F29), `oled()` undocumented 3rd param (F30), `pwm_mode` 0/1 mapping vs constants (F31), RPi::Pin 2.3609 `background_interrupt` caveat (F32). **Then re-run `pod2text lib/RPi/WiringPi.pm > README` + `perl scripts/gen-pod-md.pl` so the V27-regenerated artifacts pick up the source fixes** | `cd ~/repos/rpi-wiringpi && RPI_RELEASE_TESTING=1 RPI_POD=1 prove -bl t/500* t/505* t/510*` | POD examples valid; links resolve; POD tests pass | ⏳ |
