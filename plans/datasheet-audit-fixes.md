...
> **LAST SESSION:** 2026-06-22 — V6 done: ADS1115 `volts`/`percent` now scale by the programmed PGA FSR (read from the config MSB) instead of a constant 4.096 V (`ADS.xs` `pga_fsr()`); default-gain output unchanged so existing tests stay green. Falsified (old RED 33.6% apart / fix GREEN 0.2%). Tests: rpi-adc-ads `t/41-volts_gain.t` (PI_TEST-gated, skips w/o signal) + V6 block in rpi-wiringpi `t/142-adc_gain.t` (now 52; volts gain-consistent: gain0=gain1=0.966 V on hardware). ADS rebuilt+installed 1.03; uncommitted (user commits/releases).
> **ARCHIVE:** See datasheet-audit-fixes-archive.md for completed V1-V6

<!-- AI-STATE (terse resume state; authoritative over prose above on conflict)
PTR=V7 | Q=V7>V8>V9>V10>V11>V12>V13 | RATE=1/turn (batch only if user auths)
