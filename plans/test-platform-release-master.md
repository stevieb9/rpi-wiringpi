...
> **NEXT ACTION:** V38 — reconcile the 2 MANIFEST gaps V37 found (rpi-gpioexpander `t/02-validation.t`; rpi-wiringpi `t/440-pca9685.t` — user decides if the unreleased PCA9685 test should ship). Desk task, doable now. THEN the two big blockers before the release train: (1) **USER STRATEGIC DECISION** on the B15/rpi-i2c coupling (Discovery Fix 1 — main's t/250 now requires the 3.1803 breaking-change release the plan wanted to defer); (2) **V28** per-board suites, blocked until the fabbed boards are delivered. (V37 ✅ DONE 2026-07-10: board-independent suite + all 26 dist build+tests green.)
>
...
> **ARCHIVE:** See test-platform-release-master-archive.md for completed V tasks (V1-V5, V7-V10, V12-V27, V33-V35, V37)
...
|----|------|---------|----------|--------|
| V38 | **PR** — Reconcile the 2 MANIFEST gaps found in V37 (committed test files absent from MANIFEST → excluded from the release tarball + skipped by CPAN installers/testers): (a) rpi-gpioexpander-mcp23017 `t/02-validation.t` — add to MANIFEST (likely a plain oversight); (b) rpi-wiringpi `t/440-pca9685.t` — **USER DECIDE first**: PCA9685 is the unreleased "planned" board-1 leaf, so this may be intentionally held; add to MANIFEST only if it should ship now. | per repo: add to MANIFEST (or `make manifest`) + `ExtUtils::Manifest` fullcheck | Both MANIFESTs reconciled; no committed test silently unshipped | ⏳ **(a) DONE 2026-07-10** — added `t/02-validation.t` to rpi-gpioexpander-mcp23017 MANIFEST, fullcheck now clean (uncommitted, user commits); **(b) OPEN** — `t/440-pca9685.t` awaiting USER ship/hold decision on the unreleased PCA9685 test |
...
**Fix 1:** discovered during V37 (2026-07-10) — the board-independent suite failed only on `t/250-i2c_unit.t` (8/15). It asserts the reworked **RPi::I2C 3.1803** API (7-bit addr croak "out of range", `read_bytes` accumulation, `write_word` value-first arg order, `write_block` 32-byte cap), but the **installed** module was **3.1802** (pre-rework). Fix: built + installed local `~/repos/rpi-i2c` 3.1803 → t/250 15/15, full board-independent suite 738/738 PASS. **STRATEGIC FLAG (USER DECISION NEEDED):** those 3.1803 changes ARE exactly **B15**, which the plan files under *Explicitly NOT doing → "schedule as its own minor release AFTER the main train."* But main's committed suite (`t/250`, commit `0011bce`) now **requires 3.1803**, coupling the breaking rpi-i2c release into the main train — B15's "defer" framing is overtaken by events and must be reconciled before the release waves (either main's floor bumps to the 3.1803 release and B15 ships *with* the train, or t/250 is gated/reverted to defer 3.1803).
