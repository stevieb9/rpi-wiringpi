...
> **NEXT ACTION:** V25 — Complete the wiringpi-version-single-source plan (finalize BuildCheck in rpi-const, xt drift gate, docs; the Makefile.PL shim is already in all 25 dists) — re-verify vs the current tree first
>
> **ORDERING (revised 2026-07-09):** reordered so everything is settled before the audit/verify/release back-end. Sequence: **V25** (finalize the single-source min-version machinery + drift gate → all wiringPi-min floors provably consistent) → **V26** (audit versions/floors/Changes over the fully-settled tree) → **V27** + **V35** (user-hardware sessions; must precede V28) → **V28** (pre-release verification gate) → **V29-V32** (release waves, in const→api→leaves→main order). This supersedes the earlier "V25 last" (user delegated the call): V25 can't follow the release waves, since V28 disttests all 21 dists and V31 ships the leaves. V25's residual doesn't disturb the audit (the shim is already in place), and putting it first guarantees the drift gate has enforced floor consistency before V26 audits.
...
> **ARCHIVE:** See test-platform-release-master-archive.md for completed V tasks (V1-V5, V7-V10, V12-V24, V33, V34)
...
|----|------|---------|----------|--------|
| V25 | **PR** — Complete the wiringpi-version-single-source plan (V1-V8 there, under its own file/rules; user may authorize batching there): finalize BuildCheck in rpi-const, the xt drift gate, and docs. The Makefile.PL shim is ALREADY present in all 25 dists (verified 2026-07-09) — residual is the drift gate + docs + rpi-const finalize | `perl scripts/audit-family-buildcheck.pl --markdown` | Zero drift rows; that plan's table empty | ⏳ |
| V26 | **PR** — Version/floor/Changes AUDIT (F1, F2, F9; debate H1+H7): re-derive every claimed defect and floor from the CURRENT tree — assume staleness (three parties propagated one dead fact in the 2026-07-03 debate; note F1's rpi-pin is now 3.1803 module-side, and rpi-rtc-ds3231 0.04 UNREL is no longer empty — re-derive, don't trust the sub-item text). Two-directional floors: (a) every PREREQ floor resolvable at its release wave; (b) floors RAISED to the fix-carrying version wherever main's suite depends on fixed behavior (RPi::Pin→3.1802, RPi::EEPROM::AT24C32→1.00, each V12-V21 fix-carrier; RTC already correct at 0.03). Bump RPi::Pin module as needed; fix rpi-i2c Changes header (2.3609→current); rewrite main's stale UNREL filenames; drop-or-fill empty UNREL sections | rerun family inventory; `perl -c` each touched file | Floors resolvable at wave AND raised where the suite depends on them; Changes files coherent; audit table recorded in this plan | ⏳ |
