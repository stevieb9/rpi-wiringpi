...
> **NEXT ACTION:** Release train gated on **V28** (per-board suites — needs the fabbed boards, mostly UNDELIVERED) → then V29-V32 + V36. **B15/rpi-i2c RESOLVED** (ships with the train; all family floors bumped to repo versions). Optional desk work available NOW: remaining HW-free backlog items (B1 dac word-builder, B7 hcsr04 conversion, B8 dht11 frame, B11 pin mode_alt, B14 core mocked tests). (V37/V38/V39/V40/V41 ✅ DONE 2026-07-10.) **NEW 2026-07-11:** RPi::StepperMotor::A4988 REDESIGNED (DI transport → WiringPi::API + optional MCP23017 expander) and MPU6050/ADXL335 wired into main — integration done, install + HW-verify open → **V42** (A4988) and **V43** (gyro/accel); V36, census, F15, and the inventory row updated to match.
>
...
> **LAST SESSION:** 2026-07-11 — **A4988 redesign + three-leaf integration into main.** (1) **Redesigned RPi::StepperMotor::A4988** off the DI/pi-injected transport onto `WiringPi::API` + `RPi::Const` with an **optional MCP23017 expander** (now Pi-only; `RPi::WiringPi` is no longer merely *recommended* — the API path is a hard dep). Rewrote the dist suite + both main mirror tests: `t/354-a4988_unit.t` HW-free via a mock expander (48/48), `t/353-a4988.t` now **live through a real MCP23017** via `stepper_motor(model=>'A4988')` (gated `RPI_A4988`+`RPI_MCP23017`). (2) Added **`stepper_motor(model=>…)`** dispatch (default `'ULN2003'`; `'A4988'` → the new driver). (3) Wired the other two unconnected leaves into main: **`gyro()`** → RPi::Gyro::MPU6050 (+ live `t/358`, `RPI_GYRO`) and **`accelerometer()`** → RPi::Accelerometer::ADXL335 (+ HW-free `t/359` + live `t/360`, `RPI_ADXL335`); POD + FAQ for both. (4) Documented **expander sharing** (main FAQ + expander-lib POD) after confirming the XS does per-bit read-modify-write. Regenerated the FAQ test-table + POD→md. Added **V42** (A4988 redesign follow-through: install the redesigned 0.01 so the main tests stop skipping + HW re-verify) and **V43** (MPU6050/ADXL335 main-integration verify). **F15** annotated in place; **V36**, census, and the A4988 inventory row updated. **Uncommitted across ~/repos/{rpi-steppermotor-a4988, rpi-wiringpi, rpi-gyro-mpu6050, rpi-accelerometer-adxl335, rpi-gpioexpander-mcp23017} — user commits.** **EARLIER 2026-07-10** — ran **V40** ✅ (B5: converted 3× `exit(-1)`→`croak()` in the gpioexpander XS — a library must never terminate the caller; 27/27) + **V41** ✅ (B13: eeprom `new()` now croaks on init failure instead of blessing a broken fd=-1 object, + HW-free test; 22/22). Before that **V39** ✅ (promoted from B2): exposed the ADS scaling math (`_pga_fsr`/`_scale_volts`/`_scale_percent`) in ADS.xs + HW-free `t/06-scale.t` (18 subtests), routed voltage_c/percent_c through them; **live-confirmed on the connected 0x48** (t/40/45/50 pass under PI_TEST; pure math == live path, d=0.0000 averaged). Also this session: **B15/rpi-i2c RESOLVED** (user: ship rpi-i2c 3.1803 WITH the train) → **family-wide floor policy** — bumped all 18 stale main PREREQ_PM floors to their repo versions + **GPSD::Parse/IPC::Shareable reclassified in-family** (1.04/1.20); supersedes V26's minimal-floor audit; V32 must confirm ALL floors CPAN-indexed. **EARLIER** — ran **V37** ✅ (board-independent pre-release verification, split from V28 because boards aren't delivered): rpi-wiringpi board-independent suite **738/738** + **all 26 family dists build+test PASS**; MANIFEST fullcheck surfaced **2 real gaps → V38** (committed `t/440-pca9685.t` + `t/02-validation.t` absent from MANIFEST); worktree clean (cleaned a leaked rpi-sysinfo `SysInfo.c`). **Fix 1:** installed RPi::I2C was 3.1802 but `t/250` asserts local **3.1803** (the B15 rework) → built+installed 3.1803, suite green; **FLAGGED the B15/rpi-i2c coupling as a USER STRATEGIC DECISION** — main's committed suite now requires the breaking rpi-i2c release the plan wanted to defer (B15/Explicitly-NOT-doing is overtaken). Then ran **V38** ✅ — closed both MANIFEST gaps (added `t/02-validation.t` to rpi-gpioexpander + `t/440-pca9685.t` to main; user chose SHIP for PCA9685, verified safe — gated, skips on CPAN, no dep on the unreleased leaf); both MANIFESTs fullcheck-clean. NEXT = the B15 decision + V28 (needs boards). **EARLIER 2026-07-10 — ran V35** ✅ on real silicon. USER wired the DS3231/AT24C32 ZS-042 module to the I2C bus and declared board-2 disconnected. `i2cdetect -y 1` = **0x57 (EEPROM) + 0x68 (RTC) present**, **but 0x48 (board-2 ADS) STILL ACKs → contradicts the board-2-disconnected claim; FLAGGED to user** (harmless to V35, which is EEPROM-gated only). `RPI_BOARD=1 RPI_EEPROM=1 prove -lv t/540-eeprom_args.t t/541-eeprom_read_write_byte_croak.t t/542-eeprom_read_write_byte.t` = **166/166 PASS**; t/542 write→read round-trips (single addr 100→232 + top page 4080-4095) confirm the 1.00 **ACK-poll write cycle** returns on ACK, on real silicon. Gated EEPROM-only (NOT RPI_BOARD_4 — BMP180/OLED not wired, absent on i2cdetect). All pin-cleanup subtests green (no pin-18 mode-31 contamination this isolated run). NEXT = V28 (pre-release gate). **EARLIER 2026-07-10** — closed **V27** (USER powered board-2 rig, no servo — ADS1015 reads GPIO18 PWM on A0). Sanity-checked the rig live (0x48 ACKs; GPIO18→A0 drive test high→~2.2V / low→0V / idle→0V, confirming a ~2/3 resistive divider on the board's GPIO18 net + no RC averaging). Replaced t/425's vacuous ±40 bound with a real calibrated gate: single ADS reads are phase-dependent (0..~33 at every position, useless — a severed line reading 0 passes), but the MEAN of 100 reads tracks ideal duty within ~0.3pt (LEFT→3.0/CENTRE→7.5/RIGHT→12.75). Single-sourced `%servo_adc_windows{pi5}` + `rpi_servo_adc_window()`/`rpi_servo_adc_mean()` in RPiTest.pm; t/425 now asserts mean-in-window (LEFT[1.5,5.5]/CENTRE[5.5,10.5]/RIGHT[10.5,15.5]) + sweep gate (read≤36, peak>15). 6/6 + peak GREEN (RIGHT hit 14.27 on final run → validated the 15.5 ceiling). Same session live-proved t/400 + t/405 windows still hold. **Lone failure across t/425/400/405-in-isolation = pre-existing pin-18 mode-31 contamination (got 0/expected 31), PROVEN pre-existing via stash-compare → flagged for V28.** **Uncommitted: t/RPiTest.pm + t/425-servo.t — user commits.** (that session's NEXT was V35, now ✅ done.)
...
> **ARCHIVE:** See test-platform-release-master-archive.md for completed V tasks (V1-V5, V7-V10, V12-V27, V33-V35, V37-V41)
...
97 t/*.t, all with `# TESTDOC:` markers (generator-enforced). Board-gated:
RPI_BOARD_1 ×1 (t/335), _2 ×6, _3 ×2, _4 ×16, _5 ×4 — 29 files. **Device-gated
bench tests (not tied to any board):** t/353-a4988.t (`RPI_A4988` + `RPI_MCP23017`
— now driven through an MCP23017 expander after the 2026-07-11 A4988 redesign),
t/358-gyro.t (`RPI_GYRO`, MPU6050), t/360-adxl335.t (`RPI_ADXL335`). 65
board-independent (incl. the non-gated t/354-a4988_unit.t,
t/357-gyro_deadband_unit.t and t/359-adxl335_unit.t).
(counts updated 2026-07-11; original census re-measured in the 2026-07-03 debate, H4).
...
| RPi::StepperMotor::A4988 | — (new) | 0.01 UNREL | **NEW leaf 2026-07-10** — pure-Perl A4988 driver, DI transport; never on CPAN; installed locally; main mirror tests t/353/t/354 added; release tracked by V36. **HW owned: user bought a NEMA17 kit (1.0 A/phase, 33 N·cm).** **REDESIGNED 2026-07-11: DI transport dropped → WiringPi::API + RPi::Const with an optional MCP23017 expander (Pi-only); integrated into main via `stepper_motor(model=>'A4988')`; mirror tests t/353/t/354 rewritten — follow-through V42** |
...
| V32 | **PR** — Release wave 4: RPi::WiringPi 3.1803 (main, last): final regen_docs (also regenerates the FAQ test-table's POD→md derivatives after the 2026-07-10 A4988 rows **and the 2026-07-11 sensor rows t/358/t/359/t/360**), MANIFEST check, **ship the new main surface added 2026-07-11 — `stepper_motor(model=>…)` dispatch + `gyro()`/`accelerometer()` accessors + their POD/FAQ (V42/V43)**, verify every V26-raised floor **and the new RPi::StepperMotor::A4988 0.01 floor (V36)** is indexed on CPAN before upload, disttest on the Pi, user uploads; then watch CPAN testers across the family for a week; file any FAILs as new V rows | metacpan + testers matrix | Raised floors indexed; main live; family-wide testers PASS/NA only | ⏳ |
| V36 | **PR** — RPi::StepperMotor::A4988: NEW family leaf (2026-07-10). **HW ON HAND — user PHYSICALLY BOUGHT a NEMA17 Stepper Motor Kit (1.0 A/phase, 33 N·cm, 1.8°/200-step) + A4988 driver carrier**, so t/353 is runnable on real silicon (set VREF ≈1.0 A first). **Review + main tests DONE this session:** code reviewed reasonable/datasheet-accurate/DI-transport/HW-free (F15); dist suite 74 PASS; main mirror tests authored + green — t/354-a4988_unit.t (non-gated HW-free, 52/52) + t/353-a4988.t (gated `RPI_A4988`, live GPIO readback, 22/22, RP1 output-latch confirmed); installed locally; MANIFEST + FAQ table updated. **Release-prep PENDING (rides V31/wave-3):** replace the Dist::Mgr-boilerplate 0.01 Changes with a real entry, disttest, user uploads; then BEFORE main ships (V32) add `RPi::StepperMotor::A4988 => 0.01` to main Makefile.PL PREREQ_PM + confirm CPAN-indexed. Both main tests guard on the module being installed, so main stays green pre-release. **REDESIGNED 2026-07-11 (supersedes the DI-transport basis above):** the dist now drives through `WiringPi::API` + `RPi::Const` with an optional MCP23017 expander (Pi-only); its `Makefile.PL` prereqs changed accordingly (drop Scalar::Util; add RPi::Const/WiringPi::API). Main mirror tests rewritten (t/353 live via a real MCP23017, `RPI_A4988`+`RPI_MCP23017`; t/354 HW-free mock expander, 48/48) and reached through `stepper_motor(model=>'A4988')`. The install-gap (OLD DI 0.01 still installed → main tests skip) + HW re-verify are tracked as **V42**; the 0.01 release-prep here is otherwise unchanged (real Changes, disttest, upload; add the main PREREQ_PM edge before V32). | on-Pi `prove -l t/353-a4988.t t/354-a4988_unit.t`; metacpan post-upload | A4988 0.01 (expander redesign) live on CPAN; main prereq added + indexed; mirror tests run for installers | ⏳ |
| V42 | **PR/PC** — RPi::StepperMotor::A4988 **REDESIGN follow-through** (2026-07-11). The dist was moved off the DI/pi-injected transport onto `WiringPi::API` + `RPi::Const` with an optional MCP23017 expander (Pi-only). **DONE this session:** dist code + suite rewritten; main `stepper_motor(model=>'A4988')` dispatch added (default `'ULN2003'`); main mirror tests rewritten — t/354-a4988_unit.t (HW-free mock expander, 48/48) + t/353-a4988.t (live via a real MCP23017, `RPI_A4988`+`RPI_MCP23017`); expander-sharing documented (main FAQ + expander-lib POD). **OPEN:** the **OLD DI 0.01 build is still installed**, so both main A4988 tests currently `skip_all` via their `->can('pi')` guard — install the redesigned 0.01 and confirm they RUN; re-verify t/353 on real silicon (NEMA17 kit on hand; set VREF≈1.0 A; now driven through the expander); dist interface/Changes change rides V36's release-prep. | install redesigned dist; on-Pi `prove -l t/353-a4988.t t/354-a4988_unit.t` | Both tests RUN (not skip) + green; live readback through the expander confirmed | ⏳ 2026-07-11: **install-gap CLOSED** — redesigned A4988 0.01 installed to site_perl (`can('pi')` now 0, was 1); `t/354-a4988_unit.t` flips skip→**PASS 48/48**; full board-independent suite **778/778** green, no regressions. Also reinstalled gyro/accel/expander/main to current local code. **REMAINING:** live `t/353` through a real MCP23017 (powered rig — user). |
| V43 | **PR/PC** — RPi::Gyro::MPU6050 + RPi::Accelerometer::ADXL335 **wired into main** (2026-07-11). **DONE this session:** `gyro()` accessor (I2C MPU6050) + `accelerometer()` accessor (analog ADXL335 through an injected ADC) added to RPi::WiringPi with POD + FAQ; tests — t/358-gyro.t (live, `RPI_GYRO`), t/359-adxl335_unit.t (HW-free mock ADC, 31/31), t/360-adxl335.t (live, `RPI_ADXL335`); both accessors' dispatch verified HW-free. **OPEN:** neither leaf is installed, so t/359 `skip_all`s and the live tests need the bench parts — install both 0.01 leaves + verify on hardware. Their CPAN releases already ride **V31** (leaves) and their main floors **V32**; this row is the new main-repo integration surface those waves must ship + V28 must verify. | install leaves; on-Pi `prove -l t/357-gyro_deadband_unit.t t/358-gyro.t t/359-adxl335_unit.t t/360-adxl335.t` | Accessors documented; t/359 runs green; live gyro/accel verified | ⏳ 2026-07-11: **install-gap CLOSED** (via the V42 install pass) — MPU6050 + ADXL335 0.01 installed; `t/357` 13/13 + `t/359-adxl335_unit.t` **PASS 31/31** (both were skipping). **REMAINING:** live `t/358` (MPU6050) + `t/360` (ADXL335) on the bench (user). |
...
**Direction-(a) — all 23 prereqs resolvable at their wave:** every remaining floor (ADS 1.03, MCP3008/BMP180/DAC/DigiPot/HCSR04/LCD/OLED/SPI/Stepper 3.1801, **RPi::I2C 3.1803** [bumped 2026-07-10, Fix 1], DHT11 1.05, SysInfo 1.02, Accelerometer/Gyro/Radar 0.01) is ≤ its local module version and ships in wave 3 (leaves) before main (wave 4). No prereq floor points above what its dist will publish.
...
**SUPERSEDED 2026-07-10 (user decision — "require the repo version"):** the minimal-floor stance above is retired. Main's PREREQ_PM now pins **every family floor to its current repo/release version** (18 bumps: ADS 1.03→1.04, MCP3008/BMP180/DAC/DigiPot/HCSR04/LCD/OLED/SPI/Stepper 3.1801→3.1802, Const 1.06→1.07, DHT11 1.05→1.06, EEPROM 0.01→'1.00', GPIOExpander 1.03→1.04, Pin 3.1802→3.1803, RTC 0.03→0.04, SysInfo 1.02→1.03, WiringPi::API 3.1803→3.1804; plus RPi::I2C 3.1803 from Fix 1). Consequence: **V32 must confirm ALL these floors are CPAN-indexed** (they resolve at wave-3, before main's wave-4) — not just the previously-"raised" few. GPSD::Parse (1.03→1.04) + IPC::Shareable (1.17→1.20) were ALSO reclassified **in-family** (user 2026-07-10) and bumped to their repo versions — they now join the wave-3 leaf releases + the V32 CPAN-index check (see B21).
...
B2: → PROMOTED to V39 (2026-07-10); B2 slot retired. [was: rpi-adc-ads — expose pga_fsr + 12-vs-16-bit full-scale + VREF/percent scaling for HW-free tests (the rig chip's 12-bit path is production with zero HW-free verification).]
...
B5: → PROMOTED to V40 (2026-07-10); B5 slot retired. [was: rpi-gpioexpander-mcp23017 — getFd/_establishI2C exit(-1) → croak. Done: all 3 XS exit(-1) → croak.]
...
B13: → PROMOTED to V41 (2026-07-10); B13 slot retired. [was: rpi-eeprom-at24c32 — new() should croak/warn on eeprom_init -1 instead of fd=-1 object (TCG F6).]
...
B15: ✅ SUPERSEDED 2026-07-10 (archived Fix 1, user decision) — the rpi-i2c breaking API fixes (read_bytes accumulation TCG F7, write_word arg order TCG F8, process() arg order, write_block 32-byte cap) are IMPLEMENTED in rpi-i2c **3.1803** and now ship **WITH** the main train (not a separate post-train release): main PREREQ_PM floor bumped 3.1801→3.1803, wave-3 publishes it, V32 confirms it's CPAN-indexed before main uploads. No longer deferred/backlog.
...
B21: ✅ RECLASSIFIED 2026-07-10 (user decision) — GPSD::Parse + IPC::Shareable are now **IN-FAMILY**. Main PREREQ_PM floors bumped to repo versions (GPSD::Parse 1.04, IPC::Shareable 1.20); they join the wave-3 leaf releases and V32's CPAN-index verification like every other family floor. **Count note:** the family tallies elsewhere in this plan ("21 sub-dists", "19 leaves", "22 dists") predate this reclassification — read them as **+2** (23 sub-dists / 21 leaves / 24 dists incl. the A4988 leaf).
...
- ~~**rpi-i2c breaking API fixes inside the main train** (B15) — ship separately~~ **REVERSED 2026-07-10 (user decision):** rpi-i2c **3.1803** (the reworked API — 7-bit validation + read_bytes/write_word/write_block fixes) ships **WITH** the train; main requires 3.1803 (`t/250` asserts it). See archived Fix 1 + B15.
