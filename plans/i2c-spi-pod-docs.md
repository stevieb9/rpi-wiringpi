...
> **NEXT ACTION:** Plan essentially COMPLETE (2026-07-08). All V tasks done (V8 landed; V11 skipped); the Validation Table is empty. Remaining is optional backlog only, and NONE is HW-free actionable now: (a) chip-gated — **B11** runtime needs an MCP23017 on the bus (code done as V20; wire the chip to 0x20 to run t/05-registers.t), **B12** needs the SSD1306 OLED; (b) fetch-blocked — **B21** (bundle UM10204 once a verified copy is obtainable; the canonical NXP URL 404s). Done 2026-07-08: B2, B6, B7, B13, B14, B15, B18, B19, B20, V8; B1 declined. Uncommitted this session: `rpi-i2c` (V8), `rpi-wiringpi` (B2 FAQ.pod + plan bookkeeping), and all 14 repos' MANIFEST.SKIP/.gitignore (B18); `rpi-rtc-ds3231` (B14) already committed.
