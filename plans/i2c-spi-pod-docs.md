...
B19: rpi-dac-mcp4922 — _set() (XS) combines word-building (DAC-select via __set_dac, the data-field clear/OR) with the SPI write (_write_dac), so its register math isn't HW-free unit-testable; t/register.t can only cover the pure builders (_reg_init, __set_dac). That is why V18's clear-mask bug went uncaught. Split the word-building into a pure int-returning helper (like __set_dac) and add a HW-free test asserting the full-field clear across the 8/10/12-bit lsb values (2/4 and a stale-cache case). Discovered during V18.
