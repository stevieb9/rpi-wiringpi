...
> **NEXT ACTION:** Plan essentially COMPLETE (2026-07-08). All V tasks done (V8 landed; V11 skipped); the Validation Table is empty. Remaining is optional backlog only, and just two categories are left: (a) chip-gated — **B11** runtime needs an MCP23017 on the bus (code done as V20; wire the chip to 0x20 to run t/05-registers.t); (b) doc/packaging hygiene — B1, B2, B6, B12, B18, B20. Uncommitted this session: `rpi-i2c` (V8), `rpi-rtc-ds3231` (B14), plus `rpi-wiringpi/plans/*` bookkeeping — commit when ready.
