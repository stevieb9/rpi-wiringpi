...
> **NEXT ACTION:** Plan essentially COMPLETE (2026-07-08). All V tasks done (V8 landed; V11 skipped); the Validation Table is empty. Remaining is optional backlog only: (a) chip-gated — **B11** runtime needs an MCP23017 on the bus (code done as V20; wire the chip to 0x20 to run t/05-registers.t), **B12** needs the SSD1306 OLED; (b) HW-free doc/packaging — **B20** (rpi-pwm-pca9685 I2C ADDRESSING captions restyle), **B21** (bundle UM10204 once a verified copy is fetchable). Done 2026-07-08: B2, B6, B7, B13, B14, B15, B18, B19, V8; B1 declined. Uncommitted this session: `rpi-i2c` (V8), `rpi-wiringpi` (B2 FAQ.pod + plan bookkeeping), and all 14 repos' MANIFEST.SKIP/.gitignore (B18); `rpi-rtc-ds3231` (B14) already committed.
...
B20: rpi-pwm-pca9685 — spun off from B6/V21 (2026-07-07): the model dist's I2C ADDRESSING section captions are still lowercase, outside the DEVICE SPECIFICS / REGISTER MAP / ON THE WIRE triad B6 restyled. Restyle those captions to leading capitals for full consistency with the convention. Doc-only, non-blocking.

B21: rpi-i2c — bundle the UM10204 datasheet PDF locally under `docs/datasheet/` and repoint the DATASHEET POD at it via `F<...>`, matching the V16 convention (every other in-scope dist bundles its datasheet). Deferred during V8 (2026-07-08): the canonical NXP URL `https://www.nxp.com/docs/en/user-guide/UM10204.pdf` now 404s (verified via curl → HTTP/2 404) and no clean, provenance-verified mirror was found from here, so V8 cited UM10204 by document identity (Rev. 7.0, 1 Oct 2021) instead. When a verified copy is obtainable, bundle it + add to MANIFEST. Doc/packaging hygiene, non-blocking.
