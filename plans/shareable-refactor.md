...
> **NEXT ACTION:** V5 — run the meta-data tests on hardware (`t/03`, `t/05`, `t/110`, `t/111`) with the Pi env vars.
...
|----|------|---------|----------|--------|
| V5 | Meta data tests on hardware | (Pi) `PI_BOARD=1 RPI_OBJECT_COUNT=0 RPI_PIN_COUNT=0 prove -l t/03-meta.t t/05-checksum_uuid.t t/110-register.t t/111-metadata_multi_pi_single_script.t` | all pass | ⏳ |
| V6 | Object/pin registration + cleanup paths (Core.pm/WiringPi.pm) | (Pi) `PI_BOARD=1 RPI_OBJECT_COUNT=0 RPI_PIN_COUNT=0 prove -l t/100-identification_and_label.t t/105-pin.t t/106-pin_map.t t/150-cleanup.t` | all pass; no pins left registered after cleanup | ⏳ |
