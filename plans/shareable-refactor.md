...
> **NEXT ACTION:** None — all V tasks complete (V1–V11). The IPC::Shareable migration is done. Remaining open items are pre-existing, backend-independent backlog only: B4 (dev-scratch files), B5 (broken fatal_exit=0 tests), B6 (Pi5 alt-31 pin-status cascade), B7 (sysinfo env-format).
...
> **LAST SESSION:** V11 PASS — **migration complete (V1–V11 all done).** Fixed the migration-introduced global-destruction warnings: at exit `DESTROY → cleanup → meta_lock` died on IPC::Shareable's already-torn-down semaphore (`Shareable.pm:524`), tripping the `$SIG{__DIE__}` handler → echo + `_cleanup_handler` on undef `$self` (`WiringPi.pm:433`). Fix = a weak `%objects` registry + an `END` block that cleans live, un-`{clean}` objects **while the segment is still alive**, which also *restores* working implicit cleanup the migration had silently broken (object_count now drops 1→0 at exit). Guards: `DESTROY` + `_class_signal_handler` `return if ${^GLOBAL_PHASE} eq 'DESTRUCT'`; added `use Scalar::Util qw(weaken)`. Verified clean across V11 cmd / explicit-cleanup / two-object cases; regression batch green (t/02/03/05/100/105/110/150/153 = 465 tests, + t/112-114 cross-process death). One t/110 hiccup was a missing `RPI_PIN_COUNT` env var (identical on pristine via `git stash`), not my change. Changes file updated. Prior: V10 PASS (`--size` segment-headroom gate: realistic blob 21.9% of cap, oversized store croaks cleanly, no `size` bump), V9 (POD), V8 (ZERO-regression sweep → B5/B6/B7), V7 (cross-process t/112-114), V6 (registration/cleanup + Fix 1 `Core.pm:272`, committed `99e5010`), V5 (hardware meta), V4 (t/02 key), V3 (no-XS gate), V2 (tie-a-scalar rewrite), V1 (Makefile.PL dep swap).
