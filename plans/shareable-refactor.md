...
> **NEXT ACTION:** V11 (global-destruction warnings when no explicit cleanup) or V10 (segment-size headroom no-XS gate) — both ⏳, pick either.
...
> **LAST SESSION:** V9 PASS — Meta.pm POD updated (CRC32 key, tie-a-scalar, fixed false meta_lock internal-locking claim); gate green (perl -c + podchecker). Discovered `bin/pimeta`/`bin/pimetaerase` DO exist & work (corrected B3). New **V11** logged: global-destruction warnings when an object isn't explicitly `cleanup()`d (IPC::Shareable teardown trips `__DIE__` handler; cosmetic, data safe). Prior: V8 PASS (migration-clean) — full `make test` sweep. **Migration introduces ZERO regressions**, proven by categorizing all 37 failing files / 120 subtests: t/02 passed; every shm test fails ONLY pin-status teardown subtests (pass in isolation). Suite not literally green due to pre-existing, backend-independent issues → **B6** (Pi5 alt-31 pin-status cascade: wiringPi `pinModeAlt` can't set alt 31), **B7** (t/400-407 sysinfo env-format), **B5** (t/154 broken). Smoking gun: non-shm interrupt tests t/200-212 fail the identical trailing pin-status subtests. Fix 1 committed in `99e5010` (`Core.pm:272`). Earlier: V7 PASS (cross-process via t/112-114), V6 PASS (registration/cleanup + Fix 1), V5 PASS (hardware meta tests), V4 (t/02 key), V3 (no-XS gate), V2 (tie-a-scalar rewrite), V1 (Makefile.PL dep swap).
> **ARCHIVE:** See SHAREABLE-archive.md for completed V tasks (V1-V9), Fix 1, and B5/B6/B7
...
|----|------|---------|----------|--------|
| V11 | Global-destruction warnings when an object is not explicitly `cleanup()`d (migration-introduced; found during V9). Without cleanup, IPC::Shareable's tied-scalar teardown dies at `Shareable.pm:524` ("Can't call method op...") during global destruction, which trips `$SIG{__DIE__}` → per-uuid handler → `$self->_cleanup_handler` on an already-undef `$self` (`WiringPi.pm:433`). Cosmetic only (data safe, `destroy=>0`), but noisy. With explicit `cleanup()` there are no warnings. Investigate: guard the `__DIE__` handler against `${^GLOBAL_PHASE} eq 'DESTRUCT'`/undef `$self`, and/or an END-time cleanup of live objects. | (Pi) `perl -Ilib -e 'use RPi::WiringPi; my $p=RPi::WiringPi->new(label=>"x",shm_key=>"rpit"); $p->meta_fetch;'` | no warnings at exit | ⏳ |
...
B3: (CORRECTED — premise was wrong) `bin/pimeta` and `bin/pimetaerase` DO exist and work with the new backend (they ride the `meta_*` shim). The only possible enhancement left: `pimetaerase` currently clears segment *contents* via `meta_erase` but does not *remove* the segment itself — a true segment-removal tool could be built on `IPC::Shareable->remove` / `shm_segments`. Optional; not required by the migration.
...
B5: Pre-existing broken signal/death tests (byte-identical on master, NOT migration-related; found during V7). `t/154-sig_die_multi.t` uses `eval { die }` (caught → no cleanup fires) yet asserts pins reset to INPUT under `fatal_exit => 0`, which the module does not do (the test's own TODO: "fatal_exit => 0 doesn't clean up pins"). `t/155-sig_die.t` self-skips "fatal_exit = 0 TESTS NEED TO BE FIXED". Also `t/multi/int_slave.pl:18` triggers "Not a subroutine reference" on self-SIGINT (slave still dies; t/114 passes). Either fix the `fatal_exit => 0` cleanup path + rewrite the tests to use real process death, or formally retire them. Out of scope for the IPC::Shareable migration since the real cross-process death/cleanup is already covered green by t/112/113/114.

B6: Pi5 (BCM2712) pin-status teardown cascade in full `make test` (found during V8; backend-independent). wiringPi's `pinModeAlt` **cannot set alt mode 31** ("pinModeAlt: invalid mode 31"), so any pin whose pristine default is alt 31 (4,5,6,13,14,15,16,19,20,21,24,25 on Pi5) can't be restored by cleanup once a test touches it. In a full sweep the first test to touch each such pin knocks it permanently off-default, cascading `rpi_check_pin_status` failures (and t/107/108/109 alt-mode, t/106 pin_map readback) across all later tests. Isolated runs pass because pins are pre-reset via `pinctrl set N no` (which CAN set alt 31). Fix options: have cleanup use `pinctrl`/raw register write for alt-31 restore, OR make `rpi_check_pin_status` reset-aware, OR mark these teardown checks SKIP on Pi5.

B7: Pre-existing env-specific sysinfo test failures (t/400–407, found during V8; no shm involvement). Test regexes don't match this Pi5/OS output: `df` shows `/dev/mmcblk0p2` not `/dev/root`, swap is `/dev/zram0` not `/var/swap`, plus network/raspi-config/cpu/mem format drift. Update the parsers/expected patterns for current Raspberry Pi OS.
