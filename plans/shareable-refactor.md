...
> **NEXT ACTION:** Only **B10** remains (`t/107-alt_modes` is Pi 5-incompatible and crashes — a test-modernisation job). V1–V11 and B3–B9 are done. B9 turned out to be a real `cleanup()` bug (the alt-0/1 twin of B6) and is fixed, clearing the t/400-407 cascade. Full sweep is now **71/72 files green** (1808 subtests); the sole remaining failure is t/107 (B10).
...
B10: `t/107-alt_modes.t` is Pi 5 / RP1 incompatible and crashes (exit 255, no plan). It asserts a pin's default mode is INPUT (`0`) at line 23, but RP1 reports `31` ("no function"); it then calls `mode()` with an alt value the current `mode()` rejects ("mode param must be either 0/1/2/3"), which croaks and aborts the file. Same class as the sysinfo tests modernised in B7 (board-specific assumptions). Out of scope for B6, which explicitly excluded tests that *intentionally set* alt modes. Fix = update the test's expected defaults/alt handling for the RP1 funcsel scheme (or board-gate the legacy assertions), mirroring `rpi_board_tag`/`rpi_default_pin_config`. Straightforward test modernisation.
