...
#   (the as-drawn side is NOT curated here: render-doc.py extracts each
#   board's real caps from its .kicad_pcb via kicad-caps.py)
...
# na) from required_uf vs the caps extracted from the board .kicad_pcb files
# (kicad-caps.py), so changing a cap in KiCad clears/creates a conflict with
# no edit here at all.
BYPASS = {
...
# but are still physically on the board. The coverage audit (render-doc.py)
# counts these into the board's expected cap pool so every declared cap is
# accounted for - intent missing from a board reports as a conflict.
# board -> [value_uF, ...].
