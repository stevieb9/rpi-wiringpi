...
See C<pimetadata -h> for full details.

=head2 pimetasize

Prints to C<STDOUT> the size in bytes of the shared memory space.

Requires L<IPC::Shareable> and L<Devel::Size> to be installed.

=head2 pi-erase-metadata
...
See C<pi-erase-metadata -h> for full details.
...
hardware test platform:
    ...
    0x05  ATMega328-P IC (not always in I2C mode, so not always available)
