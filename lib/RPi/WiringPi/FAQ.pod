...
NOTE: Your user account B<must> be a member of the C<gpio> group for
...
obviously want to be doing other things while waiting for that button press.
    ...
    # LOW to HIGH. The second parameter is a code reference
    # to the handler at the bottom of this example
    ...
    # an inline (anonymous) callback works too. The callback
    # receives ($edge, $timestamp_us)
...
NOTE: As of C<WiringPi::API> 3.18, the handler B<must> be a code reference; the
older string sub-name form (eg. C<'main::handler'>) is no longer accepted.

=head3 Driving interrupt dispatch

Because callbacks now run in your own interpreter rather than from the C thread,
they fire only when your program services the interrupt. Drive that from your
main loop:
...
for you:
    ...
    # Process-global switch, so it lives on the Pi object.
...
NOTE: the per-pin C<< $pin->background_interrupt >> form shown above is not
available yet - L<RPi::Pin> (as of C<2.3608>) implements only C<set_interrupt>,
so a future C<RPi::Pin> release must ship the method first. To background many
pins in one child today, use C<< $pi->background_interrupts >> (see
L<RPi::WiringPi/background_interrupts>).
...
Every test in the suite requires the base variables C<PI_BOARD> and
C<RPI_OBJECT_COUNT> to run at all (set C<RPI_PIN_COUNT=0> as well for a clean
registration baseline); see L</Base information> below. The C<Additional env
vars> column lists only the I<extra> variables a given test needs on top of
those - if it reads C<(none)>, the base variables are sufficient. Tests marked
C<RPI_SUDO> re-run themselves under C<sudo> automatically.
...
Both C<RPI_OBJECT_COUNT> and C<RPI_PIN_COUNT> B<must be set to a numeric value>,
even if that value is C<0> (the normal case, where nothing else is registered in
shared memory). The registration tests (eg. C<t/110-register.t>) compare the
live object/pin counts found in shared memory against these variables. If a
variable is left unset, it reads as C<undef> rather than C<0>, and you'll see
spurious failures such as C<< got '0', expected undef >>. Likewise, the value
must reflect reality: setting C<RPI_OBJECT_COUNT=1> when no auto-startup object
is actually registered produces C<< got '0', expected '1' >> style failures.
When running an individual test by hand, prefix it accordingly, eg.:

    PI_BOARD=1 RPI_OBJECT_COUNT=0 RPI_PIN_COUNT=0 prove -lv t/110-register.t

C<RPI_POD> enables and disables POD tests.
...
hardware test platform:
    ...
    0x20  GPIO Expander 1
    0x21  GPIO Expander 2
    0x3c  OLED (main case)
