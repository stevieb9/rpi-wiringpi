...
C<RPI_BOARD_X> convenience block names, and the env gates that block enables.
...
shared memory). For a board test the C<Additional env vars> column lists every
gate that board's C<RPI_BOARD_X> switch turns on (setting C<RPI_BOARD_X> enables
all of them at once); for a test not tied to a board it lists only the C<RPI_*>
variables that test skips on, or C<(none)> when C<RPI_BOARD> alone suffices.
Tests marked C<RPI_SUDO> re-run themselves under C<sudo> automatically.
...
satellite board. See L</RUNNING TESTS> rig runners under C<t/scripts/> for the
per-board suites.
