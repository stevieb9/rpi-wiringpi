...
serial, you must disable bluetooth in the C</boot/firmware/config.txt> file (C</boot/config.txt> on releases before
Bookworm):
...
Every test in the suite requires C<PI_BOARD> to run at all; see L</Base
...
variables a given test needs on top of C<PI_BOARD> - if it reads C<(none)>,
C<PI_BOARD> alone is sufficient. Tests marked C<RPI_SUDO> re-run themselves under
...
C</boot/firmware/config.txt> file (C</boot/config.txt> on releases before
Bookworm) (this disabled Bluetooth on the Serial interface):
...
memory). They are not a run gate - C<PI_BOARD> decides whether the suite runs.
