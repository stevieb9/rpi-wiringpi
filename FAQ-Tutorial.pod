...
that allow you to display the last test run results on an LCD.
...
Next, run a C<bbdispatch> run against the local tester in C<--auto> mode.
Sending in C<0> to the C<--auto> flag will run the entire unit test suite every
time a commit is pushed to the master branch at Github. If no updates have been
pushed, we'll simply sleep for 60 seconds. This happens continuously.

    bbdispatch -r https://github.com/stevieb9/rpi-wiringpi -a 0
...
To make it work, you need to restart the dispatcher with the C<--rpi> flag:
