...
- [STEPPER MOTOR](#stepper-motor)
  ...
  - [Example (ULN2003)](#example-uln2003)
  - [Example (A4988)](#example-a4988)
...
The `stepper_motor` method drives a stepper motor. Two drivers are supported,
selected with the `model` parameter (named for the driver chip, the same way
`adc` and `dac` pick their part):

- `ULN2003` (the default) - [RPi::StepperMotor](https://metacpan.org/pod/RPi%3A%3AStepperMotor) driving a 28BYJ-48
unipolar motor through a ULN2003.
- `A4988` - [RPi::StepperMotor::A4988](https://metacpan.org/pod/RPi%3A%3AStepperMotor%3A%3AA4988) driving a bipolar motor through
an Allegro A4988 microstepping driver, with software microstep selection and
power-state control.
...
The A4988 is a step/direction driver with named control lines. `step` and
`dir` are mandatory; the microstep-select (`ms1`/`ms2`/`ms3`) and power
(`enable`/`sleep`/`reset`) lines are optional:
    ...
    $sm->mode('sixteenth'); # Smoother, finer microstepping
    $sm->ccw(90);

As with the ULN2003, an `expander` drives the A4988 from an MCP23017's pins
instead of the Pi's own GPIO - the `step`/`dir` (and any other) pin numbers
then name the expander's `0-15` lines:
...
`RPI_BOARD_X` convenience block names, and the `RPI_*` variables it reads.
...
shared memory). The `Additional env vars` column lists only the `RPI_*`
variables _that specific test_ reads or overrides on top of `RPI_BOARD` - if it
reads `(none)`, `RPI_BOARD` alone is sufficient. Setting the test's
`RPI_BOARD_X` switch (the `Test hardware` column) enables all of its board's
gates at once, so you needn't export each by hand. Tests marked `RPI_SUDO`
re-run themselves under `sudo` automatically.
...
order.
    ...
    353-a4988.t                                 A4988 stepper driver (live, through an MCP23017 expander) -             RPI_A4988, RPI_A4988_ADDR, RPI_A4988_DIR, RPI_A4988_ENABLE, RPI_A4988_MS1, RPI_A4988_MS2, RPI_A4988_MS3, RPI_A4988_RESET, RPI_A4988_SLEEP, RPI_A4988_STEP, RPI_MCP23017
    ...
    510-oled_unit.t                             RPi::OLED::SSD1306 unit (HW-free)                         -             (none)
    ...
    621-lcd_unit.t                              RPi::LCD unit (HW-free)                                   -             (none)
