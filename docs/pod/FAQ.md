...
**This table is generated** from the test suite by
`scripts/gen-faq-test-table.pl` (run automatically by
`scripts/gen-pod-md.pl`) - do not edit it by hand. Each row is derived from the
test file itself: its `# TESTDOC:` description marker, the board its
`RPI_BOARD_X` convenience block names, and the env gates that block enables.
...
shared memory). For a board test the `Additional env vars` column lists every
gate that board's `RPI_BOARD_X` switch turns on (setting `RPI_BOARD_X` enables
all of them at once); for a test not tied to a board it lists only the `RPI_*`
variables that test skips on, or `(none)` when `RPI_BOARD` alone suffices.
Tests marked `RPI_SUDO` re-run themselves under `sudo` automatically.

The hardware tests run against a small set of purpose-built _satellite test
boards_ that hang off the Pi (board 1, the host), each carrying the real devices
for its group of tests. The `Test hardware` column names the board a test lives
on via its convenience variable, `RPI_BOARD_X` (X = the board number). Setting
...
per-board suites.
    ...
    355-mcp23017.t                              MCP23017 GPIO expander                 RPI_BOARD_3   RPI_MCP23017, RPI_STEPPER
    356-mcp23017_unit.t                         MCP23017 unit (HW-free)                -             (none)
    400-pwm_hw_mods.t                           HW PWM sweep (read via ADC)            RPI_BOARD_2   RPI_SUDO, RPI_I2C, RPI_ADC, RPI_MCP3008, RPI_MCP4922, RPI_SERVO, RPI_SHIFTREG, RPI_DIGIPOT
    405-pwm_i2c_adc.t                           PWM/I2C/ADC integration                RPI_BOARD_2   RPI_SUDO, RPI_I2C, RPI_ADC, RPI_MCP3008, RPI_MCP4922, RPI_SERVO, RPI_SHIFTREG, RPI_DIGIPOT
    410-dac.t                                   MCP4922 DAC (read via MCP3008)         RPI_BOARD_2   RPI_SUDO, RPI_I2C, RPI_ADC, RPI_MCP3008, RPI_MCP4922, RPI_SERVO, RPI_SHIFTREG, RPI_DIGIPOT
    411-dac_unit.t                              MCP4922 DAC unit (HW-free)             -             (none)
    420-adc_samples.t                           ADS samples() averaging                -             RPI_ADC
    421-adc_gain.t                              ADS1115 gain/PGA arg                   -             RPI_ADC
    422-adc_unit.t                              RPi::ADC::ADS unit (HW-free)           -             (none)
    425-servo.t                                 Servo HW PWM (read via ADC)            RPI_BOARD_2   RPI_SUDO, RPI_I2C, RPI_ADC, RPI_MCP3008, RPI_MCP4922, RPI_SERVO, RPI_SHIFTREG, RPI_DIGIPOT
    435-shift_reg_adc.t                         74HC595 shift reg (read via ADC)       RPI_BOARD_2   RPI_SUDO, RPI_I2C, RPI_ADC, RPI_MCP3008, RPI_MCP4922, RPI_SERVO, RPI_SHIFTREG, RPI_DIGIPOT
    445-dpot.t                                  MCP4XXXX digital pot (read via ADC)    RPI_BOARD_2   RPI_SUDO, RPI_I2C, RPI_ADC, RPI_MCP3008, RPI_MCP4922, RPI_SERVO, RPI_SHIFTREG, RPI_DIGIPOT
    446-dpot_unit.t                             MCP4XXXX digipot unit (HW-free)        -             (none)
    500-oled_new.t                              OLED object creation                   RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    501-oled_string.t                           OLED draw string                       RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    502-oled_rect.t                             OLED rectangle                         RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    503-oled_dim.t                              OLED dim                               RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    504-oled_splash_screen.t                    OLED splash screen                     RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    505-oled_invert_display.t                   OLED invert display                    RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    506-oled_pixel.t                            OLED pixel                             RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    507-oled_char.t                             OLED character                         RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    508-oled_vertical_line.t                    OLED vertical line                     RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    509-oled_horizontal_line.t                  OLED horizontal line                   RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    520-oled_cleanup.t                          OLED cleanup                           RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    530-rtc.t                                   DS3231 real-time clock                 RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    531-bmp.t                                   BMP180 temp/pressure                   RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    532-rtc-bcd.t                               DS3231 BCD encode/decode (HW-free)     -             (none)
    540-eeprom_args.t                           EEPROM argument validation             RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    541-eeprom_read_write_byte_croak.t          EEPROM byte r/w error handling         RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    542-eeprom_read_write_byte.t                EEPROM byte read/write                 RPI_BOARD_4   RPI_RTC, RPI_BMP, RPI_EEPROM, RPI_OLED
    543-eeprom_validation.t                     EEPROM validation (HW-free)            -             (none)
    600-i2c_exceptions.t                        I2C exception handling                 RPI_BOARD_5   RPI_ARDUINO, RPI_SERIAL, RPI_LCD
    605-i2c.t                                   I2C read/write (Arduino)               RPI_BOARD_5   RPI_ARDUINO, RPI_SERIAL, RPI_LCD
    610-serial.t                                Serial loopback                        RPI_BOARD_5   RPI_ARDUINO, RPI_SERIAL, RPI_LCD
    620-lcd.t                                   HD44780 LCD                            RPI_BOARD_5   RPI_ARDUINO, RPI_SERIAL, RPI_LCD
...
The servo test (`t/425-servo.t`) drives a servo on GPIO 18 via hardware PWM
